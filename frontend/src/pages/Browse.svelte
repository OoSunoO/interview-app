...
{#if detailQuestion}
  ...
  <div class="detail-overlay" onclick={closeDetail} onkeydown={(e) => { if (e.key === "Escape") closeDetail(); }}>
    <div class="detail-panel" role="dialog" aria-modal="true" tabindex="-1" bind:this={detailDialog} onclick={(e) => e.stopPropagation()} onkeydown={(e) => { if (e.key === "Escape") closeDetail(); }}>
      ...
      <details class="dp-notes">
        <summary class="dp-notes-summary">
          ...
          笔记
          {#if detailNotes}
            <span class="dp-notes-badge">●</span>
          {/if}
        </summary>
        <div class="dp-notes-details">
          <textarea
            class="dp-notes-input"
            placeholder="记录你的思路或要点..."
            bind:value={detailNotes}
            oninput={saveNotes}
          ></textarea>
          {#if noteSaving}
            <span class="dp-notes-status">保存中...</span>
          {/if}
        </div>
      </details>
