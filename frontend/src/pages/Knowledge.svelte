      ...
      {#each filtered as cat}
        <div class="category-card" class:expanded={expanded[cat.name]}>
          <button class="category-header" onclick={() => toggle(cat.name)}>
            <div class="cat-left">
              <svg class="collapse-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class:rotated={expanded[cat.name]}>
            ...
            <div class="cat-right">
              ...
              <span class="cat-pct" class:green={pct(cat.done, cat.total) >= 80} class:yellow={pct(cat.done, cat.total) >= 40 && pct(cat.done, cat.total) < 80} class:red={pct(cat.done, cat.total) < 40}>{pct(cat.done, cat.total)}%</span>
            </div>
          </button>
            ...
            <div class="cat-body">
              <div class="cat-progress">
                <div class="progress-bar">
                  <div class="progress-fill" style="transform: scaleX({pct(cat.done, cat.total) / 100})"></div>
                </div>
              </div>
                    ...
                    <div class="sub-bar-wrap">
                      <div class="sub-bar">
                        <div
                          ...
                          class:yellow={pct(tag.done, tag.total) >= 40 && pct(tag.done, tag.total) < 80}
                          class:red={pct(tag.done, tag.total) < 40}
                          style="transform: scaleX({pct(tag.done, tag.total) / 100})"
                        ></div>
                      </div>
                    </div>
                    <svg class="sub-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      <polyline points="9 18 15 12 9 6"/>
                    </svg>
                  </button>
                {/each}
...
<style>
  ...
  .page-sub { font-size: 13px; color: var(--text-muted); margin-bottom: 4px; }
  ...
  .category-card.expanded { border-color: var(--accent-dim); }
  ...
  .category-header:active { background: var(--bg-surface); }

  .cat-left { display: flex; align-items: center; gap: 10px; }
  .collapse-icon { color: var(--text-dim); transition: transform 0.2s; flex-shrink: 0; }
  ...
  .cat-stat { font-size: 13px; color: var(--text-muted); }
  .cat-pct { font-size: 12px; font-weight: 700; padding: 2px 8px; border-radius: 6px; min-width: 40px; text-align: center; }
  .cat-pct.green { background: var(--success-bg); color: var(--success); }
  .cat-pct.yellow { background: rgba(251, 191, 36, 0.15); color: var(--warning); }
  .cat-pct.red { background: var(--danger-bg); color: var(--danger); }
  ...
  .progress-fill { height: 100%; background: var(--accent); border-radius: 3px; transform-origin: left; transition: transform 0.5s; }
  ...
  .sub-item:hover { border-color: var(--accent-dim); background: var(--bg-card-hover); }
  .sub-item:active { transform: scale(0.98); }

  .sub-left { min-width: 80px; flex-shrink: 0; }
  .sub-name { font-size: 14px; font-weight: 500; display: block; }
  .sub-count { font-size: 11px; color: var(--text-dim); margin-top: 2px; }

  .sub-bar-wrap { flex: 1; }
  .sub-bar { height: 6px; background: var(--border); border-radius: 3px; overflow: hidden; }
  .sub-fill { height: 100%; border-radius: 3px; transform-origin: left; transition: transform 0.4s; }
  .sub-fill.green { background: var(--success); }
  .sub-fill.yellow { background: var(--warning); }
  .sub-fill.red { background: var(--danger); }

  .sub-arrow { color: var(--text-dim); flex-shrink: 0; opacity: 0; transition: opacity 0.2s; }
  ...
  .start-cat-quiz:hover { background: rgba(96, 165, 250, 0.18); border-style: solid; }
  .start-cat-quiz:active { transform: scale(0.98); }
