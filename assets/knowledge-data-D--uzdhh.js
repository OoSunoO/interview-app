var e={基础:{category:`java_basic`,content:`## Java 语言核心特性
...
`,source:null},面向对象:{category:`java_basic`,content:`## 面向对象三大特征
...
`,source:null},String:{category:`java_basic`,content:`##String、StringBuffer、StringBuilder 对比
...
`,source:null},集合:{category:`java_collections`,content:`##集合框架概览
...
`,source:null},包装类:{category:`java_basic`,content:`## 包装类（Wrapper Class）
...
- 比较用 equals()，不用 ==（除非确定在缓存范围内）。`,source:`JavaGuide`},异常:{category:`java_basic`,content:`## Java 异常体系
...
- **资源释放**：优先使用 try-with-resources（JDK 7+），确保 AutoCloseable 资源自动关闭。`,source:`JavaGuide`},序列化:{category:`java_basic`,content:`## Java 序列化
...
- 推荐使用 JSON（Jackson/Gson）替代 Java 原生序列化——跨语言、可读性好、安全性高。`,source:`JavaGuide`},内部类:{category:`java_basic`,content:`## 内部类（Inner Class）
...
| 局部内部类 | ✅ | ✅ | 方法/作用域内 |
| 匿名内部类 | ✅ | ✅ | 表达式中 |`,source:`JavaGuide`},枚举:{category:`java_basic`,content:`## 枚举（Enum）
...
- 使用枚举实现策略模式（每个常量实现不同行为）。`,source:`JavaGuide`},Optional:{category:`java_basic`,content:`## Optional 类
...
- 与 Stream 配合效果最好：stream.filter(...).findFirst().map(...).orElse(...)。`,source:`JavaGuide`},BigDecimal:{category:`java_basic`,content:`## BigDecimal 精度控制
...
4. Java 中金额计算必须使用 BigDecimal。`,source:`JavaGuide`},引用:{category:`java_advanced`,content:`## Java 引用类型
...
- 可通过轮询队列，在对象被回收后执行清理逻辑。`,source:`JavaGuide`},数据类型:{category:`java_basic`,content:`## Java 数据类型
...
| 赋值 | 拷贝值 | 拷贝引用（浅拷贝） |
| == 比较 | 比较值 | 比较引用地址 |
`,source:`JavaGuide`},接口:{category:`java_basic`,content:`## 接口（Interface）
...
| 状态 | 不能有实例状态 | 可以有实例字段 |
`,source:`JavaGuide`},继承:{category:`java_basic`,content:`## 继承（Inheritance）
...
`,source:`JavaGuide`},抽象类:{category:`java_basic`,content:`## 抽象类（Abstract Class）
...
`,source:`JavaGuide`},静态成员:{category:`java_basic`,content:`## 静态成员（Static Members）
...
\`\`\`
- 适度使用，过度会降低代码可读性。
`,source:`JavaGuide`},重载:{category:`java_basic`,content:`## 重载（Overloading）
...
- 重载是编译期的，重写是运行期的。
`,source:`JavaGuide`},访问控制:{category:`java_basic`,content:`## 访问控制（Access Modifiers）
...
`,source:`JavaGuide`},参数传递:{category:`java_basic`,content:`## 参数传递（Parameter Passing）
...
`,source:`JavaGuide`}},t={HashMap:{category:`java_collections`,content:`##HashMap 核心要点
...
`,source:null},ConcurrentHashMap:{category:`java_collections`,content:`##ConcurrentHashMap 线程安全机制
...
- 支持高并发场景下的安全读写。
`,source:null},ArrayList:{category:`java_collections`,content:`##ArrayList 要点
...
`,source:null},LinkedList:{category:`java_collections`,content:`##LinkedList 要点
...
`,source:null},数组:{category:`java_basic`,content:`## 数组（Array）
...
`,source:`JavaGuide`},哈希:{category:`algorithm`,content:`## 哈希表（Hash Table）
...
`,source:`JavaGuide`},树:{category:`algorithm`,content:`## 树（Tree）数据结构
...
`,source:`JavaGuide`},数组:{category:`algorithm`,content:`## 数组（Array）
...
- 对差分数组求前缀和即可还原原数组。
- 典型题目：航班预订统计、拼车。
`,source:`LeetCode 官方题解 & 算法导论`},栈:{category:`algorithm`,content:`## 栈（Stack）
...
`,source:`LeetCode 官方题解 & 算法导论`},队列:{category:`algorithm`,content:`## 队列（Queue）
...
### 单调队列
- 队列内元素保持单调性，常与滑动窗口结合。
- 典型题目：滑动窗口最大值——用双端队列维护窗口内递减下标。`,source:`LeetCode 官方题解 & 算法导论`},链表:{category:`algorithm`,content:`## 链表（Linked List）
...
- put(key, value)：存在则更新并移动到头部；不存在则插入头部，若超出容量则删除尾部节点。`,source:`LeetCode 官方题解 & 算法导论`},List:{category:`java_collections`,content:`## List 接口
...
`,source:`JavaGuide`},Queue:{category:`java_collections`,content:`## Queue（队列）
...
`,source:`JavaGuide`},TreeMap:{category:`java_collections`,content:`## TreeMap / TreeSet / HashSet
...
`,source:`JavaGuide`}},n={泛型:{category:`java_basic`,content:`## 泛型（Generics）
...
\`\`\`
泛型方法独立于泛型类，可在非泛型类中定义。`,source:`JavaGuide`},反射:{category:`java_basic`,content:`## 反射（Reflection）
...
### 典型应用场景
Spring IoC 依赖注入、MyBatis ORM 映射、JUnit 测试框架、动态代理（JDK Proxy）、注解处理器等。`,source:`JavaGuide`},锁:{category:`java_advanced`,content:`## Java 锁机制详解
...
| 性能 | JDK 6+ 优化后差距很小 | 高并发场景略优 |`,source:`JavaGuide`},Stream:{category:`java_basic`,content:`## Stream API
...
- 避免在并行流中使用有状态的操作或有副作用的行为。`,source:`JavaGuide`},Lambda:{category:`java_basic`,content:`## Lambda 表达式
...
### 变量捕获
Lambda 可以访问外部变量，但被引用的局部变量必须是**隐式 final**（或等效 final）—— 即初始化后不改变值。`,source:`JavaGuide`},注解:{category:`java_basic`,content:`## 注解（Annotation）
...
- **Lombok**：@Getter、@Setter、@Builder（编译期处理生成代码）`,source:`JavaGuide`},单例:{category:`java_basic`,content:`## 单例模式（Singleton）
...
\`\`\`
- 天然防止反射攻击和序列化破坏，最简洁优雅。`,source:`JavaGuide`},位运算:{category:`java_basic`,content:`## 位运算
...
**性能优化**：乘除 2 的幂次用 << 和 >>，如 x * 2 → x << 1（JVM JIT 编译器通常会自动优化）。`,source:`JavaGuide`},设计模式:{category:`java_basic`,content:`## 设计模式概览
...
- **L**（里氏替换）：子类必须能替换父类。
- **I**（接口隔离）：接口应该小而专。
- **D**（依赖倒转）：面向接口编程而非实现。`,source:`JavaGuide`},代理:{category:`java_basic`,content:`## 代理模式（Proxy）
...
| 性能（调用） | 较好 | 更好 |
| 使用场景 | Spring AOP（有接口） | Spring AOP（无接口） |`,source:`JavaGuide`},位运算:{category:`algorithm`,content:`## 位运算（Bit Manipulation）
...
- 枚举子集：for (int mask = 0; mask < (1 << n); mask++)。`,source:`LeetCode 官方题解 & 算法导论`},设计:{category:`algorithm`,content:`## 设计题（Design）
...
5. **代码实现**：先写接口，再填充细节。`,source:`LeetCode 官方题解 & 各公司面经`}},r={JVM:{category:`java_advanced`,content:`##JVM 运行时内存区域
...
- **StackOverflowError**（非 OOM）：栈深度超出限制（递归过深）。`,source:null},GC:{category:`java_advanced`,content:`##垃圾回收核心要点
...
`,source:null},并发:{category:`java_advanced`,content:`##线程基础
...
`,source:null},线程池:{category:`java_advanced`,content:`##线程池核心要点
...
`,source:null},synchronized:{category:`java_advanced`,content:`##synchronized 核心要点
...
`,source:null},volatile:{category:`java_advanced`,content:`##volatile 核心要点
...
`,source:null},CAS:{category:`java_advanced`,content:`##CAS（Compare And Swap）核心要点
...
`,source:null},AQS:{category:`java_advanced`,content:`##AQS（AbstractQueuedSynchronizer）核心要点
...
`,source:null},IO:{category:`java_advanced`,content:`##Java IO 模型
...
`,source:null},类加载:{category:`java_advanced`,content:`## 类加载机制
...
- 典型场景：Tomcat 的 WebAppClassLoader（优先加载 Web 应用类）、SPI 机制（ServiceLoader 使用线程上下文类加载器）。`,source:`JavaGuide`},JMM:{category:`java_advanced`,content:`## Java 内存模型（JMM）
...
- StoreStore：禁止写-写重排序。
- LoadStore：禁止读-写重排序。
- StoreLoad：禁止写-读重排序（最重开销最大，volatile 写插入）。`,source:`JavaGuide`},jvm:{category:`java_advanced`,content:`## JVM 核心知识点概览
...
`,source:`JavaGuide`},concurrency:{category:`java_advanced`,content:`## 并发编程核心知识点概览
...
`,source:`JavaGuide`},JVM_内存模型:{category:`jvm`,content:`## JVM 内存模型
...
`,source:null},类加载机制:{category:`jvm`,content:`## 类加载机制
...
`,source:null},并发基础:{category:`concurrency`,content:`## 并发编程基础
...
`,source:null},JUC_工具:{category:`concurrency`,content:`## JUC（java.util.concurrent）
...
`,source:null}},i={Spring:{category:`java_advanced`,content:`##Spring 核心概念
...
`,source:null}},a={MySQL:{category:`database`,content:`##MySQL 核心要点
...
`,source:null},Redis:{category:`database`,content:`##Redis 核心要点
...
`,source:null},事务:{category:`database`,content:`##MySQL 事务要点
...
`,source:null},缓存:{category:`system_design`,content:`## 缓存（Caching）
...
`,source:`JavaGuide`},Redis_基础:{category:`redis`,content:`## Redis 核心要点
...
`,source:null}},o={操作系统:{category:`cs_basics`,content:`##操作系统核心概念
...
`,source:null},网络:{category:`cs_basics`,content:`##计算机网络核心概念
...
`,source:null},TCP:{category:`cs_basics`,content:`##TCP 核心概念
...
`,source:null},HTTP:{category:`cs_basics`,content:`##HTTP 核心要点
...
`,source:null},TCP_IP:{category:`network`,content:`## TCP/IP 协议栈
...
`,source:null},HTTP:{category:`network`,content:`## HTTP 协议
...
- 子→父：回调函数 Props
- 兄弟：提升状态到共同父组件
- 跨层级：Context API 或全局状态库`,source:`JavaGuide`},Hooks:{category:`react`,content:`## React Hooks 详解
...
2. 只能在 React 函数组件或自定义 Hook 中调用`,source:`JavaGuide`},JavaScript:{category:`frontend`,content:`## JavaScript 核心要点
...
| Symbol | 唯一值，用于私有属性名 |
| Proxy | 拦截对象操作，Vue 3 响应式基础 |`,source:`JavaGuide`},CSS:{category:`frontend`,content:`## CSS 核心要点
...
- **继承**：部分属性（color、font、line-height）自动继承父元素。`,source:`JavaGuide`}},c={Agent:{category:`agent`,content:`## AI Agent 核心概念
...
- **自我反思（Self-reflection）**：评估行动结果，调整策略，迭代改进。`,source:`JavaGuide`},LLM:{category:`ai`,content:`## 大型语言模型（LLM）核心概念
...
- **Top-p / Top-k**：采样策略，控制生成质量。`,source:`JavaGuide`},RAG:{category:`ai`,content:`## 检索增强生成（RAG）
...
| Chroma | 轻量嵌入式，适合原型开发 |
| Pinecone | 全托管云服务 |
| Weaviate | 支持混合搜索和 GraphQL |`,source:`JavaGuide`},深度学习:{category:`ai`,content:`## 深度学习基础
...
- **损失函数**：交叉熵（分类）、MSE（回归）。
`,source:`JavaGuide`},机器学习:{category:`ai`,content:`## 机器学习基础
...
`,source:`JavaGuide`},AI_基础设施:{category:`ai_infra`,content:`## AI 基础设施核心概念
...
`,source:`综合整理`}},l={Docker:{category:`devops`,content:`##Docker 核心概念
...
`,source:null},分布式:{category:`java_advanced`,content:`## 分布式系统基础
...
`,source:`JavaGuide`},安全:{category:`java_advanced`,content:`## Java 安全基础
...
`,source:`JavaGuide`},架构:{category:`system_design`,content:`## 系统架构设计
...
- **接口隔离**：不强迫调用者依赖不需要的方法。
- **无环依赖**：避免循环依赖（A→B→A）。
`,source:`JavaGuide`},微服务架构:{category:`microservices`,content:`## 微服务架构核心概念
...
`,source:`综合整理`},系统设计:{category:`system_design`,content:`## 系统设计核心要点
...
`,source:`综合整理`},消息队列:{category:`mq`,content:`## 消息队列核心概念
...
`,source:`综合整理`}},u={排序:{category:`algorithm`,content:`## 排序算法
...
- 数据量大、不要求稳定：快速排序。
- 数据范围有限：计数排序（O(n)）。
`,source:`JavaGuide`},图:{category:`algorithm`,content:`## 图（Graph）
...
- 用 DFS/BFS 进行染色，出现冲突则不是二分图。`,source:`LeetCode 官方题解 & 算法导论`},动态规划:{category:`algorithm`,content:`## 动态规划（Dynamic Programming）
...
- 两者本质相同，差别在写法。`,source:`LeetCode 官方题解 & 算法导论`},贪心:{category:`algorithm`,content:`## 贪心算法（Greedy Algorithm）
...
- **Kruskal 最小生成树**：每次选不构成环的最小边（配合并查集）。`,source:`LeetCode 官方题解 & 算法导论`},回溯:{category:`algorithm`,content:`## 回溯（Backtracking）
...
- 空间复杂度主要为递归深度 O(n)。`,source:`LeetCode 官方题解 & 算法导论`},二分搜索:{category:`algorithm`,content:`## 二分搜索（Binary Search）
...
- **二分近邻**：查找最接近 target 的元素，更新答案时记录最接近值。`,source:`LeetCode 官方题解 & 算法导论`},字符串:{category:`algorithm`,content:`## 字符串（String）
...
3. 在过程中更新答案。
- 典型题目：无重复字符的最长子串、最小覆盖子串、找到字符串中所有字母异位词。`,source:`LeetCode 官方题解 & 算法导论`},堆:{category:`algorithm`,content:`## 堆（Heap）
...
- 快速选择适用于一次性查找第 K 大/小元素，效率更高（平均 O(n)）。`,source:`LeetCode 官方题解 & 算法导论`},递归:{category:`algorithm`,content:`## 递归（Recursion）
...
- Java 不支持尾递归优化。`,source:`LeetCode 官方题解 & 算法导论`}},d={产品思维:{category:`product`,content:`## 产品思维
...
- 常见误区：把 MVP 想得太复杂。
`,source:`JavaGuide`},敏捷:{category:`project_mgmt`,content:`## 敏捷开发（Agile / Scrum）
...
- **代码评审**：确保代码质量和知识共享。
`,source:`JavaGuide`},行为面试:{category:`behavioral`,content:`## 行为面试核心方法论
...
`,source:`综合整理`},职业发展:{category:`career`,content:`## 职业发展核心要点
...
`,source:`综合整理`}},f={...e,...t,...n,...r,...i,...a,...o,...s,...c,...l,...u,...d};function p(e){return f[e]||null}function m(e){let t=Object.create(null);for(let n of e)if(n.tags)for(let e of n.tags)t[e]||(t[e]={questionIds:[],...f[e]||{category:n.category||``}}),t[e].questionIds.push(n.id);for(let[e,n]of Object.entries(f))t[e]||(t[e]={...n,questionIds:[]});return t}export{m as buildKnowledgeMap,p as getKnowledgeForTag,f as knowledgeContent};